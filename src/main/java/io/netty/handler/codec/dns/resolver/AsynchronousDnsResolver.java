...
public final class AsynchronousDnsResolver {
    ...
    private static DatagramChannel createChannel(
            ChannelFactory<DatagramChannel> channelFactory, EventLoopGroup eventLoopGroup) {
        ...
        DatagramChannel channel = channelFactory.newChannel(/*eventLoopGroup.next()*/);
        ...
        //	add first parameter for netty4.0.17.Final
        // 	channel.unsafe().register(regFuture);  // change to 
        channel.unsafe().register(eventLoopGroup.next(), regFuture);
    ...
    /*
     * modidy for netty 4.0.17.Final 还未将Inbound / Outbound 接口合并到 ChannlHandler
     * 因此参照 ChannelDuplexHandler 实现，从 SimpleChannelInboundHandler 派生并实现了 OutboundHandler 的接口方法
     */
    @ChannelHandler.Sharable
    private static final class DnsResponseHandler extends DuplexSimpleChannelHandler/*SimpleChannelInboundHandler*/<DnsResponse> {
         ...
         */
        @Override
        public void /*messageReceived*/channelRead0(ChannelHandlerContext ctx, DnsResponse response) throws Exception {
